...
begin
   ...
   Put("3. Consulta ");New_Line;
   put("4. Modificar ");new_line;
   Put("5. Salir ");New_Line;
...
----------MODIFICAR----------
procedure modificar_Lista_Reg (Cab : in Puntero) is
   cuenta:integer;
   saldo:float;
   nombre:string(1..20);
   begin
      Put("Modificar");New_Line(2);
      Put ("Ingrese cuenta de registro a modificar:");New_Line;
      get(cuenta);
      Temp:=Cab;
      if Cab /= null
         then
            for i in 1..size(A) loop
               if Cuenta=Temp.All.Info.Cuenta
                  then
                     put ("Ingrese nuevo nombre:");get(nombre);new_line;
                     put ("Ingrese nuevo saldo:");get(saldo);new_line;
                     temp.all.info.nombre:=nombre;
                     temp.all.info.saldo:=saldo;
                     write(A,B,I);
            end if;
            exit when cab=null;
            end loop;
      ...
      Put("Teclee <<enter>>");
      Skip_Line;
end modificar_Lista_Reg;
----------CONSULTA-----------
procedure consulta_Lista_Reg (Cab : in Puntero) is
   cuenta:integer;
   begin
      Put("Consulta");New_Line(2);
      Put ("Ingrese cuenta a modificar:");New_Line;
      get(cuenta);
      Temp:=Cab;
      if Cab /= null
         then
            loop
               if Cuenta=Temp.All.Info.Cuenta
                  then
                     Put(Temp.All.Info.Cuenta,0);New_Line;
                     Put(Temp.All.Info.Nombre(1..Temp.All.Info.L));New_Line;
                     Put(Temp.All.Info.Saldo,0,2,0);New_Line(3);
               end if;
            exit when cab=null;
            end loop;
         else
...
begin
   ...
   loop
      ...
      case Op is
         ...
         WHEN 3 => consulta_Lista_Reg(cabeza);
         when 4 => modificar_lista_reg(cabeza);
         when 5 => Guardar_Archivo(Cabeza);
