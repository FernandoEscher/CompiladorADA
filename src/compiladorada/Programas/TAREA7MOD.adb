...
-- MODIFICACION A SALDO DESPLIEGA TODA LA INFORMACION DEL NODO
...
procedure m_saldo (cab: in puntero) is
   mayor:puntero; -- apunta al nodo del mayor
   begin
      Temp:=Cab;
      mayor:=temp;
      while temp /= null loop
            if temp.all.info.saldo > mayor.all.info.saldo then
               mayor:=temp; -- posiciona el puntero mayor si temp tiene mayor saldo
            end if;
            temp:=temp.all.sig;
      end loop;
      New_Line(2);
      Put("CLIENTE CON MAYOR SALDO : ");New_Line(2);
      Put("Cuenta : ");
      Put(Mayor.All.Info.Cuenta,0);New_Line;
      Put("Nombre: ");
      Put(Mayor.All.Info.Nombre(1..Mayor.All.Info.L));New_Line;
      Put("Con saldo de : ");
      put(mayor.all.info.saldo,0,2,0);New_Line(3);
   end m_saldo;
